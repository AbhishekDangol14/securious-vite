...
import { ADD_ASSET_ALERT, ADD_EDIT_SOLUTION_PARTNER, ADD_PRODUCT_LIST, DELETE_ASSET_ALERT, DELETE_PRODUCT_LIST, SET_EDIT_SOLUTION_PARTNER, SET_SOLUTION_PARTNER } from './mutations.type'
...
export const solutionPartner_module = {
    ...
    actions: {
        ...
        update_solutionPartner({state}) {
            axios.put('http://127.0.0.1:8000/api/admin/solution_partners/'+state.editSolutionPartner.id,state.editSolutionPartner).then((response)=>{
                console.log(response)
            })
        },
        async [EDIT_SOLUTION_PARTNER] (context,id) {
            await axios.get('http://127.0.0.1:8000/api/admin/solution_partners/'+id+'/edit').then((response) => {
    ...
    mutations: {
        ...
        [ADD_ASSET_ALERT] (state, {asset,index,edit=null}) {
            if (edit)
                state.editSolutionPartner.solutionPartnersProducts[index].asset_alert.push(asset)
            else
