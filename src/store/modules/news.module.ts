...
export const news_module = {
  ...
  actions: {
    [STORE_NEWS](context, news) {
      ...
      axios.post(API_URL + "/admin/news", news).then((response) => {
        console.log(response);
      });
    },

    [GET_NEWS](context) {
      axios.get(API_URL + "/admin/news").then((response) => {
    ...
    [UPDATE_NEWS](context, news) {
      ...
      data.append(
        ...
        JSON.stringify(news.friendlyTranslations)
      );
      data.append("_method", "PUT");
      axios.post(API_URL + "/admin/news/" + news.id, data).then((response) => {
        console.log(response);
      });
    },
